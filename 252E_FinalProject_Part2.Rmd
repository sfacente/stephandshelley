...
**Covariates**: FIB-4 score (a validated measure used for prediction of cirrhosis, which uses age, platelet count and liver transaminases), years since FIB-4 score last measured, biologic sex, race, insurance type (mediCal vs private- a surrogate of socioeconomic status.)
...
* For the $C(t)$ variable, if a subject had no primary care or hepatology visits in the prior year ($A(t-1) = 0$) we set $C(t)$ to a 80% chance of being censored; if they had at least one visit in the prior year then we set $C(t)$ to only an 8% chance of being censored.
...
generate_data <- function(n) {
  U <- data.frame(UW = runif(n = n, min = 0, max = 1),
                  ...
                  UA1 = rgamma(n = n, shape = 0.7, scale = 3.5),
                  UL2 = rgamma(n = n, shape = 1.5, scale = 2), 
                  UA2 = rgamma(n = n, shape = 0.7, scale = 3.5),
                  UL3 = rgamma(n = n, shape = 1.5, scale = 2),
                  UA3 = rgamma(n = n, shape = 0.7, scale = 3.5),
                  UL4 = rgamma(n = n, shape = 1.5, scale = 2),
                  UA4 = rgamma(n = n, shape = 0.7, scale = 3.5),
  ...
  O <- O %>% 
    mutate(W = ifelse(UW < 1/16, 1,
           ...
           L3 = L2 + 0.1*UL3,
           A3 = round(as.numeric(UA3 + 0.1*L3),0),
           L4 = L3 + 0.1*UL4,
           A4 = round(as.numeric(UA4 + 0.1*L4),0), 
           C5 = ifelse(A4 == 0, as.numeric(UC5<0.8), as.numeric(UC5<0.08)),
           Y5 = as.numeric((UY5+0.01*A4-0.05*L4)< 0.03))
  return(O)
}
...
generate_data_intervene <- function(n, c, a1, a2, a3, a4) {
  U <- data.frame(UW = runif(n = n, min = 0, max = 1),
                  UL1 = rgamma(n = n, shape = 1.5, scale = 2),
                  UA1 = rgamma(n = n, shape = 0.7, scale = 3.5),
                  UL2 = rgamma(n = n, shape = 1.5, scale = 2), 
...
#armadillo  need better counterfacyuals here
```{r PsiF, include = FALSE}
set.seed(8675309)
SimData_none <- generate_data_intervene(1000, 0, 0, 0, 0, 0)
SimData_max <- generate_data_intervene(1000, 0, 6, 6, 6, 6)
