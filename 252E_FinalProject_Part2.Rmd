...
```{r numvisits, include = TRUE, echo =FALSE, cache = TRUE}
...
```{r HCC, iinclude = TRUE, echo = FALSE}
...
```{r fibplots, include = TRUE, echo = FALSE, cache=TRUE}
...
```{r intervene, include = TRUE, echo = FALSE}
...
```{r checking estimator performance with ltmle, include = TRUE, echo = TRUE}
B = 50
...
Unfortunately, this simulation ran for >60 hours and ultimately crashed Stephanie's laptop (without saving :/). We'd love some help on how to optimize this. Notably we keep getting an error message about rescaling the L variables (even after we thought we rescaled them), so that might be one thing to look at.
...
```{r loading in results from binary analysis, include  = FALSE, echo = FALSE, eval = TRUE}
load("O.Rdata")
...
```{r dataset analysis with binary exposure}
...
Despite our efforts to simply our analysis to limit the number of possible treatment regimes (i.e. examining the effect of 0, 1, 2, and 3 or more visits, instead of the full distribution of visit counts), this analysis is quite competationally demanding; \textit{ltmle} is quite slow, with 1 iteration of the simulation taking more than an hour to complete and 50 iterations running for >60 hours and crashing Steph's machine. We will continue to proceed and plan time for R to run analyses as needed, but any suggestions for how to streamline would be welcome!! 

## f. Next steps

Unfortunately our contact at UCSF who provided the data inadvertantly left 'date of HCC diagnosis' off of the de-identified dataset we were provided, and is having difficulty appending that variable at this point. We have just receive those diagnosis dates, and we will tune our simlation to be closer to our real data. We also need to figure out how to streamline the process of checking the estimator performance with simulation,so as mentioned, would welcome feedback. We will re-run the estimaors on our simulation data, then implement the estimators using our actual dataset. 
