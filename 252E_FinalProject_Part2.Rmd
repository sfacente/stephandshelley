...
#load("~/GitHub/stephandshelley/HCVData.Rdata") #SHELLEY
...
We are also operating with one key exclusion restriction: once someone develops the outcome [$Y(t) = 1$], we then set their $A(t+1...K)$ and $L(t+1,...,K)$ to NA for the remainder of the analysis, and set $C(K) = 1$ for that subject. <!--%%%ARMADILLO CHECK WITH STEPH%%%-->
...
```{r numvisits, include = FALSE, eval = TRUE, cache = TRUE}
...
```{r HCC, include=FALSE, eval=TRUE}
HCVData$HCC <- ifelse(is.na(HCVData$Cancer.Diagnosis.Name), 0, 1)
percentHCC <- mean(HCVData$HCC)*100
```

`r sum(HCVData$HCC)` people (`r round(percentHCC, 1)`%) developed the outcome (were diagnosed with HCC during the course of the study). At this time we are still waitingon an updated dataset that includes the year of diagnosis for each patient. <!--armadillo remember to update this in the final version! -->

**FIB-4 Score (annually)**

```{r fiblong, include = FALSE, eval = TRUE}
...
```{r fibplots, include = FALSE, eval = TRUE, cache=TRUE}
