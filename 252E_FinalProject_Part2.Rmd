...
Plot3 <- ggplot(PlottingData, aes(x = sumA, y = n)) +
geom_point() +
geom_smooth(method = "lm") +
...
#checking linearity in the binary analysis now
PlottingData <-as.data.frame(cbind(df$A1, df$A2, df$A3, df$A4, df$Y5))
PlottingData <- PlottingData %>% 
                mutate(sumA = V1+V2+V3+V4, Y5 = V5) %>% 
                dplyr::select(sumA, Y5) %>%
...
Plot4 <- ggplot(PlottingData, aes(x = sumA, y = n)) +
geom_point() +
geom_smooth(method = "lm") +
