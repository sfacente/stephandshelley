...
df$A12 <- ifelse(df$A1 == 2, 1, 0)
df$A13 <- ifelse(df$A1 == 3, 1, 0)
df$A14 <- ifelse(df$A1 == 4, 1, 0)
df$A15 <- ifelse(df$A1 == 5, 1, 0)
df$A16 <- ifelse(df$A1 == 6, 1, 0)

df$A21 <- ifelse(df$A2 == 1, 1, 0)
df$A22 <- ifelse(df$A2 == 2, 1, 0)
df$A23 <- ifelse(df$A2 == 3, 1, 0)
df$A24 <- ifelse(df$A2 == 4, 1, 0)
df$A25 <- ifelse(df$A2 == 5, 1, 0)
df$A26 <- ifelse(df$A2 == 6, 1, 0)

df$A31 <- ifelse(df$A3 == 1, 1, 0)
df$A32 <- ifelse(df$A3 == 2, 1, 0)
df$A33 <- ifelse(df$A3 == 3, 1, 0)
df$A34 <- ifelse(df$A3 == 4, 1, 0)
df$A35 <- ifelse(df$A3 == 5, 1, 0)
df$A36 <- ifelse(df$A3 == 6, 1, 0)

df$A41 <- ifelse(df$A4 == 1, 1, 0)
df$A42 <- ifelse(df$A4 == 2, 1, 0)
df$A43 <- ifelse(df$A4 == 3, 1, 0)
df$A44 <- ifelse(df$A4 == 4, 1, 0)
df$A45 <- ifelse(df$A4 == 5, 1, 0)
df$A46 <- ifelse(df$A4 == 6, 1, 0)

df <- df %>% dplyr::select(L1, A11, A12, A13, A14, A15, A16,
                           L2, A21, A22, A23, A24, A25, A26, Y2,
                           L3, A31, A32, A33, A34, A35, A36, Y3, 
                           L4, A41, A42, A43, A44, A45, A46, Y4, 
                           C5,Y5)

#estimate parameter using ltmleMSM function
results2.MSM <- ltmle::ltmleMSM(df,
                     Anodes= c("A11", "A12", "A13", "A14", "A15","A16",
