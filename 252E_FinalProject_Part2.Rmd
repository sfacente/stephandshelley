...
#Turning As into sets of binary indicators
df$A11 <- ifelse(df$A1 <= 1, 1, 0)
df$A12 <- ifelse(df$A1 <= 2 & df$A1 >1, 1, 0)
df$A13 <- ifelse(df$A1 <= 3 & df$A1 >2, 1, 0)
df$A14 <- ifelse(df$A1 <= 4 & df$A1 >3, 1, 0)

df$A21 <- ifelse(df$A2 <= 1, 1, 0)
df$A22 <- ifelse(df$A2 <= 2 & df$A2 >1, 1, 0)
df$A23 <- ifelse(df$A2 <= 3 & df$A2 >2, 1, 0)
df$A24 <- ifelse(df$A2 <= 4 & df$A2 >3, 1, 0)

df$A31 <- ifelse(df$A3 == 1, 1, 0)
df$A32 <- ifelse(df$A3 <= 2 & df$A3 >1, 1, 0)
df$A33 <- ifelse(df$A3 <= 3 & df$A3 >2, 1, 0)
df$A34 <- ifelse(df$A3 <= 4 & df$A3 >3, 1, 0)

df$A41 <- ifelse(df$A4 == 1, 1, 0)
df$A42 <- ifelse(df$A4 <= 2 & df$A4 >1, 1, 0)
df$A43 <- ifelse(df$A4 <= 3 & df$A4 >2, 1, 0)
df$A44 <- ifelse(df$A4 <= 4 & df$A4 >3, 1, 0)

df <- df %>% dplyr::select(L1, A11, A12, A13, A14, 
                           L2, A21, A22, A23, A24, Y2,
                           L3, A31, A32, A33, A34, Y3, 
                           L4, A41, A42, A43, A44, Y4, 
                           C5,Y5)

#estimate parameter using ltmleMSM function-armadillo-errors
results2.MSM <- ltmle::ltmleMSM(df,
                     Anodes= c("A11", "A12", "A13", "A14",
                               "A21", "A22", "A23", "A24",
                               "A31", "A32", "A33", "A34",
                               "A41", "A42", "A43", "A44" ),
...
Bias.MSMbeta1 <- mean(estimates_data2[,1]) - PsiF
Variance.MSMbeta1 <- var(estimates_data2[,1])
MSE.MSMbeta1 <- mean((estimates_data2[,]- PsiF)^2)
```
