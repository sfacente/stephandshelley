...
```{r Export_age_and_FIB4_for_Rena, include = FALSE}
...
```{r filter_to_adults_with_visits_and_historical_FIB4s, include = FALSE, eval = TRUE}
load("HCVData.Rdata")
...
```{r Code_for_the_DAG, include = FALSE}
...
We will implement each of these estimators using the \tt{ltmleMSM} function in the \textit{ltmle} R package. 
...
```{r datasetreconfig}
HCVDataestimate <- as.data.frame(HCVData$PtID)
HCVDataestimate$Y2 <- ifelse(HCVData$Cancer.Diagnosis.Date>'2012-12-31' | is.na(HCVData$Cancer.Diagnosis.Date), 0, 1)
HCVDataestimate$Y3 <- ifelse(HCVData$Cancer.Diagnosis.Date>'2013-12-31' | is.na(HCVData$Cancer.Diagnosis.Date), 0, 1)
HCVDataestimate$Y4 <- ifelse(HCVData$Cancer.Diagnosis.Date>'2014-12-31' | is.na(HCVData$Cancer.Diagnosis.Date), 0, 1)
HCVDataestimate$Y5 <- ifelse(HCVData$Cancer.Diagnosis.Date>'2015-12-31' | is.na(HCVData$Cancer.Diagnosis.Date), 0, 1)

HCVDataestimate$L1 <- ifelse(is.na(HCVData$FIB4_2014), HCVData$FIB4_by2014, HCVData$FIB4_2014)
HCVDataestimate$L2 <- ifelse(is.na(HCVData$FIB4_2015), HCVData$FIB4_by2015, HCVData$FIB4_2015)
HCVDataestimate$L3 <- ifelse(is.na(HCVData$FIB4_2016), HCVData$FIB4_by2016, HCVData$FIB4_2016)
HCVDataestimate$L4 <- ifelse(is.na(HCVData$FIB4_2017), HCVData$FIB4_by2017, HCVData$FIB4_2017)
...
#FINISH DATE LAST MEASURED

HCVDataestimate$A1 <- HCVData$VISIT_COUNT_2014
HCVDataestimate$A2 <- HCVData$VISIT_COUNT_2015
HCVDataestimate$A3 <- HCVData$VISIT_COUNT_2016
HCVDataestimate$A4 <- HCVData$VISIT_COUNT_2017
HCVDataestimate$A5 <- HCVData$VISIT_COUNT_2018
...
Despite our efforts to simply our analysis to limit the number of possible treatment regimes (i.e. examining the effect of 0, 1, 2, and 3 or more visits, instead of the full distribution of visit counts), this analysis isquite competationally demanding; \textit{ltmle} is quite slow, with 1 iteration of the simulation taking more than an hour to complete and 50 iterations running for more than half a day. We will continue to proceed and plan time for R to run analyses as needed, but any suggestions for how to streamline would be welcome!! 

## f. Next steps

Unfortunately our contact at UCSF who provided the data inadvertantly left 'date of HCC diagnosis' off of the last de-identified dataset we were provided, and is having difficulty appending that variable at this point. Once we receive those diagnosis dates (hopefully by Monday at the latest) we will tune our simlation to be closer to our real data, re-run the estimaors on our simulation data, then implement the iestimators using our actual dataset. 
