...
**Outcome**: The outcome is diagnosis of HCC, by the final year (2019) which occured in `r nrow(HCVData)- as.numeric(summary(HCVData$Cancer.Diagnosis.Source)["NA's"])` patients. 
...
### ii. Our DAG
```{r DAG, include = TRUE, echo = FALSE, out.width= "80%"}
...
```{r numvisits, include = TRUE, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
...
`r sum(HCVData$HCC)` people (`r round(HCCpercent, 1)`%) developed the outcome (were diagnosed with HCC during the course of the study). The figure below shows the number of people diagnosed with HCC in each year, and cumulatively, throughout the study period (\textbf{note} that 2019 is an incomplete year!)

```{r HCCplots, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.width=11, fig.height=5.5}
...
FIB-4 scores in this dataset range of 0.181 to 45.956 , with an IQR of 1.198-2.872 (i.e. the FIB-4 score of 45 is a substantial outlier). `r cirrhosis` FIB-4 scores are >3.25 throughout all years of follow-up on all patients, indicating likely cirrhosis and increased risk of HCC at those timepoints.  There are `r outliers` instances where a patient's FIB-4 score was calculated to be greater than 9.

The figure below presents the distribution of FIB-4 scores in each year.

```{r fibplots, include = TRUE, echo = FALSE, cache=TRUE, warning = FALSE, message = FALSE}
...
* \textbf{Gender}- this will be a categorical variable, likely with three categories (man, woman and non-binary). Based on a prior (small) study of HCV patients at UCSF (Burman et al 2016), it is expected that the cohort will be approximately 70% men.

* \textbf{Race}- this will be a categorical variable. Based on a prior (small) study of HCV patients at UCSF (Burman et al 2016), it is expected that the cohort will be approximately 40% White, 20% Latinx, 30% Black and 10% other races.

* \textbf{SES}- we are using insurance type (MediCal or private) as a marker of SES status, which will be a categorical variable.
...
```{r FIB4 Histograms, include = TRUE, echo = FALSE, fig.height = 5, fig.width = 11}
...
```{r Hist of Visits, include = TRUE, echo = FALSE, message = FALSE, fig.height = 4}
...
#Armadillo!-Shelley I don't understand what this is 
...
```{r checking estimator performance with ltmle and 4 levels, include = TRUE, echo = TRUE, eval = FALSE}
# 
# B = 50
...
```{r singlesim, include = FALSE, eval = TRUE, echo = TRUE }
...
# B = 100
# n= 1000
# estimates_SimParameterCheckBinary <- matrix(nrow = B, ncol = 3)
# colnames(estimates_SimParameterCheck) <- c("Gcomp", "IPTW",  "TMLE")
# 
...
# sum.results2.MSM.g
# 
# estimates_SimParameterCheckBinary[b,] <- c(sum.results2.MSM.g$cmat[2,1],
#                          sum.results2.MSM.iptw$cmat[2,1],
#                          sum.results2.MSM.tmle$cmat[2,1])
# print(b)
# 
# }
# 
 save(estimates_SimParameterCheckBinary, file ="estimates_SimParameterCheckBinary112319_100iterations.Rdata")


load("estimates_SimParameterCheckBinary112319_100iterations.Rdata")
estimates_SimParameterCheckBinary<- as.data.frame(estimates_SimParameterCheckBinary )


Bias.g <- mean(estimates_SimParameterCheckBinary[,1]) - PsiF
Variance.g <- var(estimates_SimParameterCheckBinary[,1])
MSE.g <- mean((estimates_SimParameterCheckBinary[,1]- PsiF)^2)

Bias.iptw <- mean(estimates_SimParameterCheckBinary[,2]) - PsiF
Variance.iptw <- var(estimates_SimParameterCheckBinary[,2])
MSE.iptw <- mean((estimates_SimParameterCheckBinary[,2]- PsiF)^2)

Bias.tmle <- mean(estimates_SimParameterCheckBinary[,3]) - PsiF
Variance.tmle <- var(estimates_SimParameterCheckBinary[,3])
MSE.tmle <- mean((estimates_SimParameterCheckBinary[,3]- PsiF)^2)
...
```{r datasetreconfig, include = TRUE, eval = TRUE, warning = FALSE, message = FALSE, echo = TRUE}
...
Plot3 <- ggplot(PlottingData, aes(x = sumA, y = (sumY/number))) +
geom_point() +
geom_smooth(method = "lm") +
ggtitle("Expected Probability of HCC dependant on \n How many Visits Occured Each Year")  +
xlab("Total Number of Years with Visits 2015-2018")+ 
ylab("Proportion of Patient Diagnosed with HCC")

#This is checking for linearity based on number of visits (up to 3 annually) among our observed data
Plot3
```
...
```{r dataset analysis with binary exposure, include = TRUE, cache = TRUE, warning = FALSE, message = FALSE}
...
geom_point() +
geom_smooth(method = "lm") +
ggtitle("Expected Probability of HCC dependant on \n Whether a Visit Occured Each Year")  +
xlab("Total Number of Years with Visits 2015-2018")+ 
ylab("Expected Probability of HCC")

#This is checking for linearity based on whether visits occured annually among our observed data
Plot4
```
...
If we use the TMLE estimate (an arbitrary choice at this point given that we could not complete our simulation and compare performance of the various estimators), this means that for every additional primary care or hepatology visit (up to three annually), patients are are `r round(tmlebeta, 1)`% less likely to develop HCC than people who have one fewer primary care or hepatology visits in the five years under study, adjusting for liver cirrhosis.

***

Implementing \textit{ltmle} on our observed data produced the following estimates for the $\beta$ on $\bar{A}(t) = \bar{a}(t)$ where $a \in \{0,1+\}$ hepatology or primary care visits in each timepoint (i.e. per year) - a binary outcome at each A(t):

| G-Comp | IPTW | TMLE |
| ------ | ------ | ------ |
| `r round(estimates_Obin[1], 3)` | `r round(estimates_Obin[2], 3)` | `r round(estimates_Obin[3], 3)` |

If we use the TMLE estimate, this means that over the five years under study, patients are `r round(tmlebeta_bin, 1)`% less likely to develop HCC each year that they had at least one primary care or hepatology visit, compared to patients who had no primary care of hepatology visits that year, adjusting for liver cirrhosis.
