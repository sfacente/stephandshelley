...
library(pacman)
p_load(tidyverse, readxl, skimr, knitr, xslx, lubridate, MASS, ggplot2, ggpubr, data.table, mice, survival, car)
...
fit_weib1 <- survreg(Surv(YEAR, HCC) ~ VISIT_COUNT + ABD_IMAGING + FIB4 + race.ethnicity + Sex + PayorType, 
data=HCVData1_long, dist="weibull")
summary(fit_weib1)

# alpha estimates
aw1 <- c(coef(fit_weib1), fit_exp1$icoef[2])
...
gw <- c("-a2/exp(lscale)","-a3/exp(lscale)","-a4/exp(lscale)", "-a5/exp(lscale)", "-a6/exp(lscale)", "-a7/exp(lscale)", "-a8/exp(lscale)", "-a9/exp(lscale)") #functions of the regression parameters from AFT -> PH
...
row.names(weib1) <- c("VISIT_COUNT", "ABD_IMAGING", "FIB4", "race.ethnicityB", "race.ethnicityL", "race.ethnicityO", "SexMale", "PayorTypeMediCal")

# #COX, to verify what I did is right because wow
# cox1 <- coxph(Surv(YEAR, HCC) ~ VISIT_COUNT + ABD_IMAGING + FIB4 + race.ethnicity + Sex + PayorType, data=HCVData1_long, method="efron")
# summary(cox1)
# coxcoef <- coef(cox1)
# coxCI <- confint(cox1)
