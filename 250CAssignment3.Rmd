...
\]
*where* $p$ *is the shape parameter,* $log(p * t^{p-1}) + \beta_0$ *is the log-baseline hazard, and* $\boldsymbol{x}\beta$ *is the vector of coefficients times the vector of covariate values. Each individual $\beta$ within the $\beta$ vector is the log hazard ratio for a one unit change in that variable, with Weibull distribution.*
...
% Steph copied and then edited table here so original version remains if we need to discuss further
