...
\]
*where* $p$ *is the shape parameter,* $log(p * t^{p-1}) + \beta_0$ *is the log-baseline hazard, and* $\boldsymbol{x}\beta$ *is the log HR.*
...
------- | ------------- | ------------- | ------------- | ------------- | ------------- | -------------
<18.5 | \tt{`r round(exp(coxcoef[1]),3)`} (\tt{`r round(exp(coxCI[1,1]),3)`}, \tt{`r round(exp(coxCI[1,2]),3)`}) | \tt{`r round(coxSE[1],3)`} | \tt{`r round(exp(expPHcoef[1]),3)`} (\tt{`r round(exp(expPHCI[1,1]),3)`}, \tt{`r round(exp(expPHCI[1,2]),3)`}) | \tt{`r round(expPHSE[1],3)`} | \tt{`r round(exp(weibPHcoef[1]),3)`} (\tt{`r round(exp(weibPHCI[1,1]),3)`}, \tt{`r round(exp(weibPHCI[1,2]),3)`}) | \tt{`r round(weibPHSE[1],3)`} 
18.5 - <25 | 1 | - | 1 | - | 1 | -
25 - <30 | \tt{`r round(exp(coxcoef[2]),3)`} (\tt{`r round(exp(coxCI[2,1]),3)`}, \tt{`r round(exp(coxCI[2,2]),3)`}) | \tt{`r round(coxSE[2],3)`} | \tt{`r round(exp(expPHcoef[2]),3)`} (\tt{`r round(exp(expPHCI[2,1]),3)`}, \tt{`r round(exp(expPHCI[2,2]),3)`}) | \tt{`r round(expPHSE[2],3)`} | \tt{`r round(exp(weibPHcoef[2]),3)`} (\tt{`r round(exp(weibPHCI[2,1]),3)`}, \tt{`r round(exp(weibPHCI[2,2]),3)`}) | \tt{`r round(weibPHSE[2],3)`} 
$\ge$ 30 | \tt{`r round(exp(coxcoef[3]),3)`} (\tt{`r round(exp(coxCI[3,1]),3)`}, \tt{`r round(exp(coxCI[3,2]),3)`})| \tt{`r round(coxSE[1],3)`} | \tt{`r round(exp(expPHcoef[3]),3)`} (\tt{`r round(exp(expPHCI[3,1]),3)`}, \tt{`r round(exp(expPHCI[3,2]),3)`}) | \tt{`r round(expPHSE[3],3)`} | \tt{`r round(exp(weibPHcoef[3]),3)`} (\tt{`r round(exp(weibPHCI[3,1]),3)`}, \tt{`r round(exp(weibPHCI[3,2]),3)`}) | \tt{`r round(weibPHSE[3],3)`} 
...
*From this we can see that the p-value for* $H_0 : p = 1$ *(exponential) vs.* $H_A : \sigma \neq 1$ *is essentially zero; therefore we reject the exponential and select the Weibull model.*
...
*The time ratio (TR) for individuals with BMI > 30 compared to those with BMI 18.5-<25 is* $e^{`r round(aw[4],3)`}$ = .781.

*This means that being obese (BMI > 30) multiplies survival time by by .781, or decreases survival time by about 22%.*
...
------- | ------------- | ------------- | ------------------- | ------------------- 
18.5 - <25 | 1 | 1 | 1 | 1
25 - <30 | \tt{`r round(exp(coef(cs.cvd[1])),3)`} (\tt{`r round(exp(cIcs.cvd[1]),3)`}, \tt{`r round(exp(cIcs.cvd[5]),3)`}) | \tt{`r round(exp(coef(cs.other[1])),3)`} (\tt{`r round(exp(cIcs.other[1]),3)`}, \tt{`r round(exp(cIcs.other[5]),3)`}) | \tt{`r round(exp(as.vector(sd.cvd$coef[1])),3)`} (.396, 3.47)| \tt{`r round(exp(as.vector(sd.other$coef[1])),3)`} (.153, 1.11) 
$\ge$ 30 | \tt{`r round(exp(as.vector(coef(cs.cvd))[2]),3)`} (\tt{`r round(exp(cIcs.cvd[2]),3)`}, \tt{`r round(exp(cIcs.cvd[6]),3)`}) | \tt{`r round(exp(as.vector(coef(cs.other))[2]),3)`} (\tt{`r round(exp(cIcs.other[2]),3)`}, \tt{`r round(exp(cIcs.other[6]),3)`}) | \tt{`r round(exp(as.vector(sd.cvd$coef[2])),3)`} (1.22, 8.94) | \tt{`r round(exp(as.vector(sd.other$coef[2])),3)`} (.070, .453) 
