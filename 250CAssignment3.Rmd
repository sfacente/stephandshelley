...
*The time ratio (TR) for individuals with BMI > 30 compared to those with BMI 18.5-<25 is* $e^{`r round(aw[4],3)`}$ = .781. *This means that being obese (BMI > 30) multiplies survival time by by .781, or decreases survival time by about 22%.*
...
--------- | ------------------- | ------------------- | ------------------ | ------------------ |
18.5 - <25 | 1 | 1 | 1 | 1 |
25 - <30 | \tt{`r round(exp(coef(cs.cvd)[1]),2)`} (\tt{`r round(exp(cIcs.cvd[1]),2)`}, \tt{`r round(exp(cIcs.cvd[5]),2)`}) | \tt{`r round(exp(coef(cs.other)[1]),2)`} (\tt{`r round(exp(cIcs.other[1]),2)`}, \tt{`r round(exp(cIcs.other[5]),2)`}) | \tt{`r round(exp(as.vector(sd.cvd$coef[1])),2)`} (.40, 3.5)| \tt{`r round(exp(as.vector(sd.other$coef[1])),2)`} (.15, 1.1) |
$\ge$ 30 | \tt{`r round(exp(as.vector(coef(cs.cvd))[2]),2)`} (\tt{`r round(exp(cIcs.cvd[2]),2)`}, \tt{`r round(exp(cIcs.cvd[6]),2)`}) | \tt{`r round(exp(as.vector(coef(cs.other))[2]),2)`} (\tt{`r round(exp(cIcs.other[2]),2)`}, \tt{`r round(exp(cIcs.other[6]),2)`}) | \tt{`r round(exp(as.vector(sd.cvd$coef[2])),2)`} (1.2, 8.9) | \tt{`r round(exp(as.vector(sd.other$coef[2])),2)`} (.07, .45) |
