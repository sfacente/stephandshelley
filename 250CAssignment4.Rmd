...
# Code cigarettes per day as a restricted quadratic spline. 
stroke.rqs <- coxph(Surv(timestrk, stroke) ~ age + rqs20 + rqs40, data=finaldata.stroke, ties="efron")

# Using the code below, plot predicted risk score ($exp(xˆ\beta)$) at median age for each model, side-by-side:
...
k30 <- matrix(c((x1-x0), ((x1-spl1)^2 - (x1-spl3)^2 - (x0-spl1)^2), ((x1-spl2)^2 - (x1-spl3)^2)), 1)
...
\\
\mathbf{age \times \beta_1} &\ is \ age \ in \ years, \ meaning \ that \ if \ e \ is \ raised \ to \ the \ \beta_1 \ power, \ this \ is \ the \ hazard \ ratio\\
...
\\
\mathbf{cigsperday \times \beta_2} &\ is \ the \ number \ of \ cigarettes \ the \ participant \ consumed \ per \ day, \ meaning \ that \ if \ e \ is \ raised \ to\\
...
&\ to \ those \ one \ year \ younger, \ holding \ constant \ the \ other \ variables \ in \ the \ model.\\
\\
\mathbf{cigsperday \times \beta_2} &\ is \ the \ number \ of \ cigarettes \ the \ participant \ consumed \ per \ day, \ meaning \\
...
\\
\mathbf{(s^2_{20} - s^2_{60})} &\ is \ the \ quadratic \ spline \ term \ for \ one \ pack \ a \ day \ minus \ the \ uppermost \ quadratic \ spline \ term \\
...
&\ behavior \ in \ the \ model \ due \ to \ sparse \ data.\\ 
\\
\mathbf{(s^2_{40} - s^2_{60})} &\ is \ the \ quadratic \ spline \ term \ for \ two \ packs \ a \ day \ minus \ the \ uppermost \ quadratic \ spline \ term \\ 
...
&\ model, \ disallowing \ erratic \ behavior \ in \ the \ model \ due \ to \ sparse \ data.\\ 
\end{aligned}
\]
...
*Based on the results of this test (p-value = 0.0005), we would choose the restricted quadratic spline model and express cigarette smoking using the spline knots as described in question 1, where the model parameters are defined.*
...
*Coefficient vector:* (\tt{`r k.HR11divHR01`})
