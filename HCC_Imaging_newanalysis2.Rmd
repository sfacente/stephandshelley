...
generate_data  <- function(n) {
  ...
  O <- O %>% 
    mutate(W = ifelse(UW < w1, 1,
           ...
           Y3  = ifelse(Y2==0, as.numeric(UY3 < 0.04*plogis(Lb2 - La2 - A2)),1),
           ...
           Y4  = ifelse(Y3==0, as.numeric(UY4 < 0.04*plogis(Lb3 - La3 - A3)),1),
           ...
           Y5  = ifelse(Y4==0, as.numeric(UY5 < 0.04*plogis(Lb4 - La4 - A4)),1)
           )
  return(O)
}
...
SL.library <- c("SL.glm", "SL.bayesglm", "SL.mean") ## armadillo
...
O$A1 <- HCVData$ABD_IMAGING_YN_2015
O$A2 <- HCVData$ABD_IMAGING_YN_2016
O$A3 <- HCVData$ABD_IMAGING_YN_2017
O$A4 <- HCVData$ABD_IMAGING_YN_2018

O$La1 <- ifelse(HCVData$VISIT_COUNT_2015>0,1,0)
O$La2 <- ifelse(HCVData$VISIT_COUNT_2016>0,1,0)
O$La3 <- ifelse(HCVData$VISIT_COUNT_2017>0,1,0)
O$La4 <- ifelse(HCVData$VISIT_COUNT_2018>0,1,0)

O$Lb1 <- HCVData$FIB4_by2015
O$Lb2 <- HCVData$FIB4_by2016
O$Lb3 <- HCVData$FIB4_by2017
O$Lb4 <- HCVData$FIB4_by2018

O$Lb1lm <- HCVData$YearsSinceFib4_2015
O$Lb2lm <- HCVData$YearsSinceFib4_2016
O$Lb3lm <- HCVData$YearsSinceFib4_2017
O$Lb4lm <- HCVData$YearsSinceFib4_2018

O$SES <- HCVData$PayorType
O$Sex <- HCVData$Sex
O$Race <- HCVData$race.ethnicity
  
#O$C5 <- ifelse(O$A5 == 0, 1, 0)  armadillo
  
#remove NAs for now, until we impute -- armadillo
  O <- subset(O, !is.na(O$SES))
  O <- subset(O, !is.na(O$Race))
  
  O <- O %>% dplyr::select(SES, Race, Sex, La1, Lb1, A1, 
                           La2, Lb2, A2, Y2, 
                           La3, Lb3, A3, Y3, 
                           La4, Lb4, A4, Y4, 
                           Y5)
...
tmle_SL <- summary(results, "tmle")
iptw_SL <- summary(results, "iptw")
gcomp_SL <- summary(results.gcomp, "gcomp")

tmle_ATE <- exp(tmle_SL$effect.measures$ATE$estimate); tmle_ATE
iptw_ATE <- exp(iptw_SL$effect.measures$ATE$estimate); iptw_ATE
gcomp_ATE <- exp(gcomp_SL$effect.measures$ATE$estimate); gcomp_ATE
...
If we use the G-computation estimate, this means that over the five years under study, patients with no cirrhosis at baseline were `r round(gcomp_ATE, 1)`% less likely to develop HCC by year 5 when they had abdominal imaging each year for the 4 previous years, compared to patients who had no abdominal imaging, when controlling for liver cirrhosis, number of primary care or hepatology visits, sex, race, and SES. **This represents a number needed to treat (NNT) of 92**: <!--`r NNT`**: for every `r NNT`-->for every 92 people who receive annual abdominal imaging despite no evidence of cirrhosis at baseline, 1 case of HHC could be prevented over 5 years.
