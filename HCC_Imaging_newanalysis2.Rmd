...
#First just exploring our data distrubutions so our sims can match
#Because A is the number imaging occurences in a year, A1 is imaging in 2015, A2 is in 2016...
...
#Because L_b is fib4 score, L_b1 is visits in 2015,...
...
generate_data  <- function(n) {
  ...
  O <- O %>% 
    mutate(W = ifelse(UW < w1, 1,
           ...
           Y2 = as.numeric(UY2 < 0.04*plogis(Lb1 - La1 - A1)),
           ...
           Y3  = ifelse(Y2==0, as.numeric(UY3 < 0.04*plogis(Lb2 - La2 - A2)),NA),
           ...
           Y4  = ifelse(Y3==0, as.numeric(UY4 < 0.04*plogis(Lb3 - La3 - A3)),NA),
           ...
           Y5  = ifelse(Y4==0, as.numeric(UY5 < 0.04*plogis(Lb4 - La4 - A4)),NA)
           )
  return(O)
}

#These are just to explore the simulation to make sure it's tuned
#SimData <- generate_data(1000)
#skimr::skim(SimData)
```

 Causal Question: How would the counterfactual probability of getting HCC differ by the end of the 5 year follow up under an intervention to get at least once a year every year for four years?
 
 The target causal parameter is 

$\psi^F (\mathcal{P}_{U,X}) = E_{U,X}(Y_{\bar{a}=1(5)}-Y_{\bar{a}=0(5)})$ 

the average treatment effect, assuming that all patients had the intervention (imaging) every year ($Y_{\bar{a}=1}$) and the outcomes if all patients did NOT have the intervention at any time point ($Y_{\bar{a}=0}$). This looks at having had the outcome up until time 5, based on the Y(5) notation.

```{r Intervene on Our Data structure}
...
generate_data_intervene  <- function(n, a) {
 ...
 mutate(W = ifelse(UW < w1, 1,
           ...
           Lb2 = ifelse(Y2==0, as.integer(Lb1 + 0.1*ULb2), NA),
           A2  = ifelse(Y2==0, a, NA),
           Y3  = ifelse(Y2==1, 1, ifelse(Y2==0, as.numeric(UY3 < 0.04*plogis(Lb2 - La2 - A2)),NA)),
           
           La3 = ifelse(Y3==0, La2 + 0.1*ULa3, NA),
           Lb3 = ifelse(Y3==0, as.integer(Lb2 + 0.1*ULb3), NA),
           A3  = ifelse(Y3==0, a, NA),
           Y4  = ifelse(Y3==1, 1, ifelse(Y3==0, as.numeric(UY4 < 0.04*plogis(Lb3 - La3 - A3)),NA)),
           
           La4 = ifelse(Y4==0, La3 + 0.1*ULa4, NA),
           Lb4 = ifelse(Y4==0, as.integer(Lb3 + 0.1*ULb4), NA),
           A4  = ifelse(Y4==0, a, NA),
           Y5  = ifelse(Y4==1, 1, ifelse(Y4==0, as.numeric(UY5 < 0.04*plogis(Lb4 - La4 - A4)),NA))
           )
  return(O)
}
##Updated above so that if Y(t-1)==1, Y(t)==1 armadillo
...
```{r ltmle run on our simulation with SuperLearner}

SL.library <- c("SL.glm", "SL.bayesglm", "SL.mean") ##

results_sim0 <- ltmle::ltmle(SimDataIntervene.0,
...
results_sim0.gcomp <- ltmle::ltmle(SimDataIntervene.0,
                     Anodes= c("A1", "A2", "A3", "A4"),
                     Lnodes = c("La1", "Lb1", "La2", "Lb2", "La3", "Lb3", "La4", "Lb4"),
                     Ynodes = c("Y2","Y3", "Y4", "Y5"),
                     abar = c(0,0,0,0),
...
results_sim1 <- ltmle::ltmle(SimDataIntervene.1,
                     Anodes= c("A1", "A2", "A3", "A4"),
                     Lnodes = c("La1", "Lb1", "La2", "Lb2", "La3", "Lb3", "La4", "Lb4"),
                     Ynodes = c("Y2","Y3", "Y4", "Y5"),
                     abar = c(1,1,1,1),
...
results_sim1.gcomp <- ltmle::ltmle(SimDataIntervene.1,
                     Anodes= c("A1", "A2", "A3", "A4"),
                     Lnodes = c("La1", "Lb1", "La2", "Lb2", "La3", "Lb3", "La4", "Lb4"),
                     Ynodes = c("Y2","Y3", "Y4", "Y5"),
                     abar = c(1,1,1,1),
...
sim0tmle_SL <- summary(results_sim0, "tmle")
sim1tmle_SL <- summary(results_sim1, "tmle")
sim0iptw_SL <- summary(results_sim0, "iptw")
sim1iptw_SL <- summary(results_sim1, "iptw")
sim0gcomp_SL <- summary(results_sim0.gcomp, "gcomp")
sim1gcomp_SL <- summary(results_sim1.gcomp, "gcomp")

simtmle_ATE <- sim1tmle_SL$estimate - sim0tmle_SL$estimate; simtmle_ATE
simiptw_ATE <- sim1iptw_SL$estimate - sim0iptw_SL$estimate; simiptw_ATE
simgcomp_ATE <- sim1gcomp_SL$estimate - sim0gcomp_SL$estimate; simgcomp_ATE
...
# EY1_ltmle_t_SL <- ltmle(data = ObsData, Anodes = Anodes, 
#                         Lnodes = Lnodes, Ynodes = Ynodes, 
#                         abar = 1, variance.method = "ic", 
#                         SL.library = SL.library)
# #Obtain estimates under A1 = 1 using G-comp and IPTW
# EY1_ltmle_g_SL <- ltmle(data = ObsData, Anodes = Anodes, 
#                         Lnodes = Lnodes, Ynodes = Ynodes, 
#                         abar = 1, variance.method = "ic", 
#                         gcomp = TRUE, SL.library = SL.library)
# #Store the estimates
...
# 
# ```{r Estimating additive treatment}
# #Obtain estimates under A1 = 1 using TMLE and IPTW
# EY1_ltmle_t_add <- ltmle(data = ObsData, Anodes = Anodes, 
#                          Lnodes = Lnodes, Ynodes = Ynodes, 
#                          abar = list(1,0), variance.method = "ic",
#                          SL.library = SL.library)
# #Obtain estimates under A1 = 1 using G-comp and IPTW
# EY1_ltmle_g_add <- ltmle(data = ObsData, Anodes = Anodes, 
#                          Lnodes = Lnodes, Ynodes = Ynodes, 
#                          abar = list(1,0), variance.method = "ic",
#                          gcomp = TRUE, SL.library = SL.library)
# #Print the summary
