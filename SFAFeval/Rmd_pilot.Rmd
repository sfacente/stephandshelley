...
```{r datasets}
#This chunk you will want to manually edit each time you use this report so that you are using the appropriate datasets
...
#When I first attempted to combine the datasets, it errored because this SVR12ReTx variable was a character variable in the Q3 dataset but a logical variable in all the other quarters, so I have made it a logical variable too
...
dataset<- full_join(combineQ1, combineQ2) %>% 
                            full_join(combineQ3) %>%
                            full_join(combineQ4) #you can add additional piped rows here to join more datasets
```
...
print(as.data.frame(rART_by_BRP))
```
...
print(as.data.frame(rART_by_race))
```
...
Plot1a_race <- Plot1aracedata %>% ggplot(aes(x=FiscalYearandQuarter, y=value, group=raeth, color = raeth)) + 
                        geom_line() +
                        geom_point() +
                        #geom_text(data = subset(Plot1aracedata, FiscalYearandQuarter == FYQMax),
                                  #aes(label = raeth, colour = raeth), hjust = -.1, show.legend = FALSE) + 
                        #took this line out because the labels overlap too much
