...
*We use specialized models for survival analyses because they deal with time; time-to-event must be defined as the probability of an event happening at any particular time* **t** *given that the event has not already happened <u>prior</u> to that time* **t**.
...
*The summary of our analysis to assess the proportional hazards assumption for this dataset found no significant interactions between time and our exposure variable (smoking at baseline), nor age or any levels of education (p-values for the coefficients on the interaction of time with smoking, age, and educational levels 2, 3, and 4 were* \tt{`r round(pvalues[7],2)`}, \tt{`r round(pvalues[12],2)`},  \tt{`r round(pvalues[9],2)`}, 
\tt{`r round(pvalues[10],2)`}, \fontfamily{cmr}\itshape\selectfont{and} \tt{`r round(pvalues[11],2)`}, \fontfamily{cmr}\itshape\selectfont{respectively). Therefore, we can conclude the the proportional hazards assumption has not been violated for these variables.}
