...
To be fixed (TBF): add limitations
...
TBF: add an interpretation of the 75th percentile estimate
...
*We use specialized models for survival analyses because they deal with person-time. However, the reason that survival models are specifically better than something like poisson that also accounts for person time is that in survival models, time-to-event must be defined as the probability of an event happening at any particular time* **t** *given that the event has not already happened <u>prior</u> to that time* **t**. *Said differently, these models then account for censoring.*
...
TBF: these should all be switched back to betas (not the fit coefficients)
...
*In the adjusted model, smokers have roughly a 40% increased risk of mortality during the 24 years of follow-up as nonsmokers, when adjusting for sex and educational level. While this parameter estimate is a hazard ratio, it corresponds to a rate ratio, a common measure of association in epidemiologic research.*
