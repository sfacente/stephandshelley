...
cox.timeixn.frmgham <-coxph(Surv(timedth_yrs, death)~ cursmoke + sex + as.factor(educ) + tt(as.integer(cursmoke)) + 
...
*The summary of our analysis to assess the proportional hazards assumption for this dataset found no significant interactions between time and our exposure variable (smoking at baseline), nor sex or any levels of education (p-values for interaction of time with smoking, sex, and educational levels 2, 3, and 4 were* \tt{`r pvalues[6]`}, \tt{`r pvalues[7]`},  \tt{`r pvalues[8]`}, 
\tt{`r pvalues[9]`}, \fontfamily{cmr}\itshape\selectfont{and} \tt{`r pvalues[10]`}, \fontfamily{cmr}\itshape\selectfont{respectively). Therefore, we can conclude the the proportional hazards assumption has not been violated.}

\fontfamily{cmr}\itshape\selectfont{If there had been noted violations with confounder variables, we could have accounted for that by including time interactions with that variable in our model, or alternatively by estimating a stratified Cox proportional hazards model (if the variable were categorical). If the violation were in our exposure variable, we would use a model including the time interactions and report time-specific effects (i.e.} $HR(t) = e^{\beta}$ \fontfamily{qag}\selectfont\textsuperscript{+} $^\gamma$ \fontfamily{qag}\selectfont\textsuperscript{x} $^t$).
