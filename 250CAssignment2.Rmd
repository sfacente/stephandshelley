...
*Using the equation from the previous question, we can calculate that the log-hazard for smokers (holding other covariates constant) is* \texttt{`r a`} *and the comparable log-hazard for non-smokers is* \texttt{`r b`}. *The hazard ratio generated via the model specified in the previous question is* \texttt{`r c`}, *which is the anti-log of the* $\beta_1$ *coefficient on the smoking indicator (with the* $\beta_1$ *coefficient* = \texttt{`r d`}*). This hazard ratio is exactly equal to the log-hazard of smokers divided by the log-hazard of non-smokers:* 
\[
\frac{\texttt{`r a`}}{\texttt{`r b`}} = \texttt{`r c`}
\]
...
------------ | ------------- | ------------------ | ------------------- | ------------------ | ------------------
No   | \ \ \tt{`r py.smoke.pretty[1,1]`} | \ \ \ \ 44,440.4 | *ref* | *ref*
Yes  | \ \ \tt{`r py.smoke.pretty[2,1]`} | \ \ \ \ 46,675.2 | 0.917 (0.9872, 1.295) | 1.0085 (.9093, 1.1184) |
