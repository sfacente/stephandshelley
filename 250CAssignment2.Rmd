...
*Using XXXX, we see that the log-hazard for smokers (holding other covariates constant) is* \texttt {'r a'} *and the comparable log-hazard for non-smokers is* \texttt {'r b'}. *The hazard ratio generated via the model specified in the previous question is* \texttt {'r c'}, *which is the anti-log of the* $\beta_1$ *coefficient on the smoking indicator (with the* $\beta_1$ *coefficient* = \texttt {'r d'}*). This hazard ratio is exactly equal to the log-hazard of smokers divided by the log-hazard of non-smokers:* 
\[
