...
#use the numberof events/person time in each group to estimate lambda, the rate parameter
...
coxph.frmgham.adj <-coxph(Surv(timedth_yrs, death)~cursmoke + sex + as.factor(educ), data=frmgham_recoded,
ties="efron")
...
frmgham_recoded$cursmoke2 <-as.integer(frmgham_recoded$cursmoke) 
 cox.timeixn.frmgham <-coxph(Surv(timedth_yrs, death)~ cursmoke + sex 
                          + as.factor(educ) + tt(cursmoke2) + 
                            tt(as.integer(sex)) + tt(as.integer(educ==2)) +
...
$ln[h(t|x)] = ln[h_0(t)] + x_1\beta_1 + x_2\beta_2 + x_3\beta_3 + x_4\beta_4 + x_5\beta_5$
...
* $x_4$ is an indicator corresponding to educational status category 4

*using the results from our model, the equation then becomes:*

$ln[h(t|x)] = ln[h_0(t)] +$ \texttt{`r round(as.numeric(coxph.frmgham.adj$coefficients[1]),3)`} $x_1 +$ \texttt{`r round(as.numeric(coxph.frmgham.adj$coefficients[2]),3)`} $x_2 +$ \texttt{`r round(as.numeric(coxph.frmgham.adj$coefficients[3]),3)`} $x_3 +$  \texttt{`r round(as.numeric(coxph.frmgham.adj$coefficients[4]),3)`} $x_4 +$  \texttt{`r round(as.numeric(coxph.frmgham.adj$coefficients[5]),3)`} $x_5$
...
*Thus, the hazard ratio generated via the model specified in the previous question is:*  
...
*which is the anti-log of the* $\beta_1$ *coefficient on the smoking indicator (with the* $\beta_1$ *coefficient* = \texttt{`r round(as.numeric(coxph.frmgham.adj$coefficients[1]),3)`}*). This hazard ratio is exactly equal to the log-hazard of smokers divided by the log-hazard of non-smokers:* 
...
*We have done an analysis with a model that includes the interaction with time for each of our variables, such that the model is now specified by:*
$$
ln[h(t|x)] = ln[h_0(t)] + x_1\beta_1 + x_2\beta_2 + x_3\beta_3 + x_4\beta_4 + x_5\beta_5 + x_1t\beta_6 + x_2t\beta_7 + x_3t\beta_8 + x_4t\beta_9 + x_5t\beta_10
$$

*where $\beta_6$ through $\beta_10$ are the coefficients on the interaction terms interacting each independant variable with time. For all these variables, the hazard ratios generated in our model are very close to one, with confidence intervals that span one, suggesting that no time-dependant confounding exits. Thus, we can assume that the proportionality of our hazards holds.*
